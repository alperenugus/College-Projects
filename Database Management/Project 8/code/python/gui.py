...
class LoginWindow:
    def __init__(self, window):
        ...
        self.ok_button = Button(
            self.button_frame, text='OK', command=self.ok_action)
        self.ok_button.grid(column=0, row=0)

        self.cancel_button = Button(
            self.button_frame, text='Cancel', command=quit)
...
class Application:
    def __init__(self, window, db):
        ...
        self.search_cb = Combobox(self.search_frame, values=(
            'Artist', 'Album'), font=self.font)
        ...
        self.search_button = Button(
            self.search_frame, text='Search', command=self.search_action)
        ...
        self.results_lb = Listbox(
            self.results_frame, height=10, width=80, font=self.font, exportselection=0)
        ...
        self.add_artist_button = Button(
            self.button_frame, text='Add Artist', command=self.show_add_artist)
        self.add_artist_button.grid(row=0, column=0)
        self.add_album_button = Button(
            self.button_frame, text='Add Album', command=self.show_add_album)
        self.add_album_button.grid(row=0, column=1)
        self.edit_album_button = Button(
            self.button_frame, text='Edit Album', command=self.show_edit_album, state=DISABLED)
        self.edit_album_button.grid(row=0, column=2)
        self.delete_album_button = Button(
            self.button_frame, text='Delete Album', command=self.remove_album, state=DISABLED)
    ...
    def show_add_artist(self):
        ...
        self.add_artist_entry = Entry(
            self.edit_frame, width=40, font=self.font)
        self.add_artist_entry.grid(row=1, column=1)
        save = Button(self.edit_frame, text='Save',
                      command=self.save_new_artist)
        save.grid(row=1, column=2)

    def save_new_artist(self):
        if len(self.add_artist_entry.get()) == 0:
            messagebox.showerror('Error Saving Artist',
    ...
    def show_add_album(self):
        ...
        self.artist_cb = Combobox(
            self.edit_frame, values=artist_list, font=self.font)
    ...
    def show_edit_album(self):
        ...
        self.year_entry.grid(row=3, column=1)
        save = Button(self.edit_frame, text='Save',
                      command=self.save_edited_album)
        save.grid(row=4, column=0, columnspan=2)
    ...
    def save_edited_album(self):
        if len(self.album_entry.get()) == 0:
            ...
            self.update_album(album_id, self.album_entry.get(),
                              self.year_entry.get())
...
###################
    ...
    def insert_album(self, artist_name, album_name, year):
        ...
        try:
            self.cursor.execute(query, (artist_name,))
            artist_id = self.cursor.fetchall()[0][0]
        except pg8000.Error as e:
            messagebox.showerror('Database error', str(e))
            return None
        ...
        try:
            self.cursor.execute(query, (artist_id, album_name, year, ))
            self.db.commit()
        except pg8000.Error as e:
            messagebox.showerror('Database error', str(e))
            return None

        print('Save new album: ' + artist_name + ' - ' + album_name +
              ' (' + year + ')')
    ...
    def update_album(self, album_id, album_name, year):
        ...
        print('Update album: ' + str(album_id) + ': ' + album_name +
              ' (' + year + ')')
