...
class MainActivityTest {
    ...
    @Test
    fun seekBarDisplayed(){
        onView( withId(R.id.playback_speed_seek_bar) ).check(matches( isDisplayed() ))
    }

    @Test
    fun playbackSpeedTextDefault() {
        val defaultString = mainActivityTestRule
            .activity
            .resources
            .getString( R.string.playback_speed_label, 100 )
        onView( withId(R.id.playback_text_view) )
            .check( matches( withText(defaultString) ) )
    }

    @Test
    fun playbackSpeedText150(){

        val stringAt150 = mainActivityTestRule
            .activity
            .resources
        ...
        onView( withId(R.id.playback_text_view) )
            .check( matches( withText(stringAt150) ) )
    }

    @Test
    fun playbackSpeedText25(){
        val stringAt25 = mainActivityTestRule
            .activity
            .resources
            .getString( R.string.playback_speed_label, 25 )

        onView(withId(R.id.playback_speed_seek_bar)).perform(setProgress(25))

        onView( withId(R.id.playback_text_view) )
            .check( matches( withText(stringAt25) ) )
    }
