...
int Game::betRound(Player *p1, Player *p2)
{
	...
	while( true )
	{
        assert( m_pot > 0 ) ;  // make sure pot is positive

        // get the bet
        betAmount = player[curPlyr]->getBet(player[1 - curPlyr]->getHand().getVisible(),
                m_betHistory, bet2player, canRaise, m_pot) ;
		...
		// if player folded
		if( betAmount < bet2player ) {
			return player[1 - curPlyr]->getId() ;
		}  // end if

		// adjust the pot
		m_pot += betAmount ;
		player[curPlyr]->addChips(-betAmount ) ;

		// if call or check
		if( bet2player == betAmount ) {
		    // if call, exit
		    if( betCount > 0 ) {
		        return -1 ;
		    }
		    // else a check do nothing
		}
		// else a raise
		else {
		    raises++ ;
        ...
        // add bet to history
        Bet bet( betAmount, player[curPlyr]->getId() ) ;
        m_betHistory.addBet( bet ) ;

        // process next player
		curPlyr = 1 - curPlyr ;
		bet2player = betAmount - bet2player ;
		betCount++ ;
	} // end while
